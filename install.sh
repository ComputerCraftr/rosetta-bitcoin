...
usage() {
  this=$1
  cat <<EOF
$this: download pre-compiled Docker images for coinbase/rosetta-cari 
...
execute() {
  ...
  docker tag "rosetta-cari:${TAG}" "rosetta-cari:latest"
  log_info "loaded rosetta-cari:${TAG} and tagged as rosetta-cari:latest"
  rm -rf "${tmpdir}"

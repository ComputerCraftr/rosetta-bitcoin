...
import (
	...
	cariUtils "github.com/coinbase/rosetta-cari/utils"
...
const (
	...
	// genesis block of the cari blockchain for polling
	genesisBlockIndex = 0
...
const (
	// https://cari.org/en/developer-reference#getblock
	requestMethodGetBlock requestMethod = "getblock"

	// https://cari.org/en/developer-reference#getblockhash
	requestMethodGetBlockHash requestMethod = "getblockhash"

	// https://cari.org/en/developer-reference#getblockchaininfo
	requestMethodGetBlockchainInfo requestMethod = "getblockchaininfo"

	// https://developer.cari.org/reference/rpc/getpeerinfo.html
	requestMethodGetPeerInfo requestMethod = "getpeerinfo"

	// https://developer.cari.org/reference/rpc/pruneblockchain.html
	requestMethodPruneBlockchain requestMethod = "pruneblockchain"

	// https://developer.cari.org/reference/rpc/sendrawtransaction.html
	requestMethodSendRawTransaction requestMethod = "sendrawtransaction"

	// https://developer.cari.org/reference/rpc/estimatesmartfee.html
	requestMethodEstimateSmartFee requestMethod = "estimatesmartfee"

	// https://developer.cari.org/reference/rpc/getrawmempool.html
...
const (
	...
	// returned in CARI blocks to be milliseconds.
	timeMultiplier = 1000

	// rpc credentials are fixed in rosetta-cari
	// because we never expose access to the raw carid
...
// Client is used to fetch blocks from carid and
// to parse CARI block data into Rosetta types.
//
// We opted not to use existing CARI RPC libraries
...
// NewClient creates a new CARI client.
func NewClient(
	baseURL string,
	genesisBlockIdentifier *types.BlockIdentifier,
	currency *types.Currency,
...
// ParseBlock returns a parsed cari block given a raw cari
...
// https://caricore.org/en/doc/0.20.0/rpc/blockchain/pruneblockchain
func (b *Client) PruneBlockchain(
	...
	// Parameters:
	//   1. Height
	// https://developer.cari.org/reference/rpc/pruneblockchain.html#argument-1-height
...
// function mirrors the behavior of a similar commit in cari-core.
//
// Source: https://github.com/cari/cari/commit/ab91bf39b7c11e9c86bb2043c24f0f377f1cf514
...
func (b *Client) parseTransactions(
	...
	logger := cariUtils.ExtractLogger(ctx, "client")
...
func (b *Client) parseTxOperations(
	...
	for networkIndex, input := range tx.Inputs {
		if cariIsCoinbaseInput(input, txIndex, networkIndex) {
...
func (b *Client) parseOutputTransactionOperation(
	...
	// If we are unable to parse the output account (i.e. carid
...
func (b *Client) getInputTxHash(
	...
	if cariIsCoinbaseInput(input, txIndex, inputIndex) {
		return "", -1, false
	}
...
// cariIsCoinbaseInput returns whether the specified input is
...
func cariIsCoinbaseInput(input *Input, txIndex int, inputIndex int) bool {
...
// parseOutputAccount parses a cariScriptPubKey and returns an account
...
// post makes a HTTP request to a CARI node
func (b *Client) post(
	ctx context.Context,

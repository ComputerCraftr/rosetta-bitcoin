...
import (
	...
	mocks "github.com/coinbase/rosetta-cari/mocks/indexer"
...
func TestIndexer_Pruning(t *testing.T) {
	...
	cfg := &configuration.Configuration{
		...
		GenesisBlockIdentifier: cari.MainnetGenesisBlockIdentifier,
...
func TestIndexer_Transactions(t *testing.T) {
	...
	for i := int64(0); i <= 1000; i++ {
		...
		for j := 0; j < 5; j++ {
			...
			scriptPubKey := &cari.ScriptPubKey{
				ASM: coinIdentifier,
			}
				...
				Operations: []*types.Operation{
					{
						...
						Status: types.String(cari.SuccessStatus),
	...
	go func() {
		for {
			...
			if currBlock.BlockIdentifier.Index == 1000 {
				...
				allCoins := []*types.Coin{}
				expectedPubKeys := []*cari.ScriptPubKey{}

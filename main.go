...
import (
	...
	"github.com/coinbase/rosetta-cari/indexer"
...
func startOnlineDependencies(
	...
	g *errgroup.Group,
) (*cari.Client, *indexer.Indexer, error) {
	client := cari.NewClient(
		cari.LocalhostURL(cfg.RPCPort),
		cfg.GenesisBlockIdentifier,
		cfg.Currency,
	)

	g.Go(func() error {
		return cari.StartCARId(ctx, cfg.ConfigPath, g)
	})
...
func main() {
	...
	if signalReceived {
		logger.Fatalw("rosetta-cari halted")
	}

	if err != nil {
		logger.Fatalw("rosetta-cari sync failed", "error", err)
	}
}

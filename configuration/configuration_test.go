	...
	tests := map[string]struct {
		...
		"all set (mainnet)": {
			...
			cfg: &Configuration{
				...
				Params:                 cari.MainnetParams,
				Currency:               cari.MainnetCurrency,
		...
		"all set (testnet)": {
			...
			cfg: &Configuration{
				...
				GenesisBlockIdentifier: cari.TestnetGenesisBlockIdentifier,
				Port:                   1000,
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if test.err != nil {
				...
				test.cfg.CARIdPath = path.Join(newDir, "carid")

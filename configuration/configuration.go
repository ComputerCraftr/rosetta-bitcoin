...
const (
	...
	// Mainnet is the CARI Mainnet.
	Mainnet string = "MAINNET"

	// Testnet is CARI Testnet3.
	Testnet string = "TESTNET"

	// mainnetConfigPath is the path of the CARI
	// configuration file for mainnet.
	mainnetConfigPath = "/app/cari-mainnet.conf"

	// testnetConfigPath is the path of the CARI
	// configuration file for testnet.
	testnetConfigPath = "/app/cari-testnet.conf"
	...
	mainnetRPCPort = 31814
	testnetRPCPort = 51475

	// min prune depth is 288:
	// https://github.com/cari/cari/blob/ad2952d17a2af419a04256b10b53c7377f826a27/src/validation.h#L84
	pruneDepth = int64(10000) //nolint

	// min prune height (on mainnet):
	// https://github.com/cari/cari/blob/62d137ac3b701aae36c1aa3aa93a83fd6357fde6/src/chainparams.cpp#L102
...
func LoadConfiguration(baseDirectory string) (*Configuration, error) {
	...
	case Online:
		...
		config.CARIdPath = path.Join(baseDirectory, caridPath)
		if err := ensurePathExists(config.CARIdPath); err != nil {
			return nil, fmt.Errorf("%w: unable to create carid path", err)
		}
	case Offline:
		config.Mode = Offline
	case "":
	...
	case Mainnet:
		config.Network = &types.NetworkIdentifier{
			Blockchain: cari.Blockchain,
			Network:    cari.MainnetNetwork,
		}
		config.GenesisBlockIdentifier = cari.MainnetGenesisBlockIdentifier
		config.Params = cari.MainnetParams
		config.Currency = cari.MainnetCurrency
	...
	case Testnet:
		config.Network = &types.NetworkIdentifier{
			Blockchain: cari.Blockchain,
			Network:    cari.TestnetNetwork,
		}
		config.GenesisBlockIdentifier = cari.TestnetGenesisBlockIdentifier
		config.Params = cari.TestnetParams
		config.Currency = cari.TestnetCurrency
